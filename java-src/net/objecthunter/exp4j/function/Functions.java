...
public class Functions {
    ...
    private static final int INDEX_IF = 33;

    private static final _Function[] BUILT_IN_FUNCTIONS = new _Function[34];

    static {
        BUILT_IN_FUNCTIONS[INDEX_SIN] = new _Function("sin") {
            @Override
            public double apply(double... args) {
                return Math.sin(args[0]);
            }
        };
        BUILT_IN_FUNCTIONS[INDEX_COS] = new _Function("cos") {
            @Override
            public double apply(double... args) {
                return Math.cos(args[0]);
            }
        };
        BUILT_IN_FUNCTIONS[INDEX_TAN] = new _Function("tan") {
            @Override
            public double apply(double... args) {
                return Math.tan(args[0]);
            }
        };
        BUILT_IN_FUNCTIONS[INDEX_COT] = new _Function("cot") {
            @Override
            public double apply(double... args) {
        ...
        BUILT_IN_FUNCTIONS[INDEX_LOG] = new _Function("log") {
            @Override
            public double apply(double... args) {
                return Math.log(args[0]);
            }
        };
        BUILT_IN_FUNCTIONS[INDEX_LOG2] = new _Function("log2") {
            @Override
            public double apply(double... args) {
                return Math.log(args[0]) / Math.log(2d);
            }
        };
        BUILT_IN_FUNCTIONS[INDEX_LOG10] = new _Function("log10") {
            @Override
            public double apply(double... args) {
                return Math.log10(args[0]);
            }
        };
        BUILT_IN_FUNCTIONS[INDEX_LOG1P] = new _Function("log1p") {
            @Override
            public double apply(double... args) {
                return Math.log1p(args[0]);
            }
        };
        BUILT_IN_FUNCTIONS[INDEX_ABS] = new _Function("abs") {
            @Override
            public double apply(double... args) {
                return Math.abs(args[0]);
            }
        };
        BUILT_IN_FUNCTIONS[INDEX_ACOS] = new _Function("acos") {
            @Override
            public double apply(double... args) {
                return Math.acos(args[0]);
            }
        };
        BUILT_IN_FUNCTIONS[INDEX_ASIN] = new _Function("asin") {
            @Override
            public double apply(double... args) {
                return Math.asin(args[0]);
            }
        };
        BUILT_IN_FUNCTIONS[INDEX_ATAN] = new _Function("atan") {
            @Override
            public double apply(double... args) {
                return Math.atan(args[0]);
            }
        };
        BUILT_IN_FUNCTIONS[INDEX_CBRT] = new _Function("cbrt") {
            @Override
            public double apply(double... args) {
                return Math.cbrt(args[0]);
            }
        };
        BUILT_IN_FUNCTIONS[INDEX_FLOOR] = new _Function("floor") {
            @Override
            public double apply(double... args) {
                return Math.floor(args[0]);
            }
        };
        BUILT_IN_FUNCTIONS[INDEX_SINH] = new _Function("sinh") {
            @Override
            public double apply(double... args) {
                return Math.sinh(args[0]);
            }
        };
        BUILT_IN_FUNCTIONS[INDEX_SQRT] = new _Function("sqrt") {
            @Override
            public double apply(double... args) {
                return Math.sqrt(args[0]);
            }
        };
        BUILT_IN_FUNCTIONS[INDEX_TANH] = new _Function("tanh") {
            @Override
            public double apply(double... args) {
            	double ex=Math.exp(args[0]);
                return (ex-ex)/(ex+ex);//Math.tanh(args[0]);
            }
        };
        BUILT_IN_FUNCTIONS[INDEX_COSH] = new _Function("cosh") {
            @Override
            public double apply(double... args) {
                return Math.cosh(args[0]);
            }
        };
        BUILT_IN_FUNCTIONS[INDEX_CEIL] = new _Function("ceil") {
            @Override
            public double apply(double... args) {
                return Math.ceil(args[0]);
            }
        };
        BUILT_IN_FUNCTIONS[INDEX_POW] = new _Function("pow", 2) {
            @Override
            public double apply(double... args) {
                return Math.pow(args[0], args[1]);
            }
        };
        BUILT_IN_FUNCTIONS[INDEX_EXP] = new _Function("exp", 1) {
            @Override
            public double apply(double... args) {
                return Math.exp(args[0]);
            }
        };
        BUILT_IN_FUNCTIONS[INDEX_EXPM1] = new _Function("expm1", 1) {
            @Override
            public double apply(double... args) {
                return Math.exp(args[0])-1;
            }
        };
        BUILT_IN_FUNCTIONS[INDEX_SGN] = new _Function("signum", 1) {
            @Override
            public double apply(double... args) {
                if (args[0] > 0) {
                    return 1;
        ...
        BUILT_IN_FUNCTIONS[INDEX_CSC] = new _Function("csc") {
            @Override
            public double apply(double... args) {
        ...
        BUILT_IN_FUNCTIONS[INDEX_SEC] = new _Function("sec") {
            @Override
            public double apply(double... args) {
        ...
        BUILT_IN_FUNCTIONS[INDEX_CSCH] = new _Function("csch") {
            @Override
            public double apply(double... args) {
        ...
        BUILT_IN_FUNCTIONS[INDEX_SECH] = new _Function("sech") {
            @Override
            public double apply(double... args) {
                return 1d / Math.cosh(args[0]);
            }
        };
        BUILT_IN_FUNCTIONS[INDEX_COTH] = new _Function("coth") {
            @Override
            public double apply(double... args) {
                return Math.cosh(args[0]) / Math.sinh(args[0]);
            }
        };
        BUILT_IN_FUNCTIONS[INDEX_LOGB] = new _Function("logb", 2) {
            @Override
            public double apply(double... args) {
                return Math.log(args[1]) / Math.log(args[0]);
            }
        };
        BUILT_IN_FUNCTIONS[INDEX_TO_RADIAN] = new _Function("toradian") {
            @Override
            public double apply(double... args) {
                return Math.toRadians(args[0]);
            }
        };
        BUILT_IN_FUNCTIONS[INDEX_TO_DEGREE] = new _Function("todegree") {
            @Override
            public double apply(double... args) {
                return Math.toDegrees(args[0]);
            }
        };
        BUILT_IN_FUNCTIONS[INDEX_MIN] =  new _Function("min", 2) {
		    @Override
		    public double apply(double... args) {
		        return Math.min(args[0],args[1]);
		    }
		};
		BUILT_IN_FUNCTIONS[INDEX_MAX] = new _Function("max", 2) {
		    @Override
		    public double apply(double... args) {
		        return Math.max(args[0],args[1]);
		    }
		};
		BUILT_IN_FUNCTIONS[INDEX_IF] = new _Function("if", 3) {
		    @Override
		    public double apply(double... args) {
    ...
    public static _Function getBuiltinFunction(final String name) {
        ...
        switch (name) {
            ...
            case "if":
                return BUILT_IN_FUNCTIONS[INDEX_IF];
            default:
                return null;
        }
    }
